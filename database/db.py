...
class Database:
    """
    Класс управления подключениями SQLite.
    Спроектирован так, чтобы при масштабировании его можно было заменить на PostgreSQL.
    """
    ...
    async def init_tables(self):
        ...
        await self.connection.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS listings (
                ...
                condition TEXT DEFAULT 'new',  -- new, used, good
        ...
        # Миграция: добавляем колонку `language`, если её ещё нет
        try:
        ...
        # Миграция: создаём таблицу user_reports, если её ещё нет
        try:
            await self.connection.execute("""
                CREATE TABLE IF NOT EXISTS user_reports (
                    id INTEGER PRIMARY KEY,
        ...
        # Миграция: добавляем колонку `condition`, если её ещё нет
        try:
            await self.connection.execute(
                "ALTER TABLE listings ADD COLUMN condition TEXT DEFAULT 'new'"
            )
            await self.connection.commit()
            logger.info("Migration: added 'condition' column to listings")
        except Exception:
            pass  # Колонка уже существует
...
async def close_db():
    """Закрывает глобальный экземпляр базы данных."""
    global _db
    if _db is not None:
